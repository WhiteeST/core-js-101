 ...
 *
 * @example
 *   getPolynom(2,3,5) => y = 2*x^2 + 3*x + 5  18+9+5
...
function getPolynom(...args) {
  ...
  function countPolynom(x) {
    let power = argArray.length - 1;
    return argArray.reduce((acc, val) => {
      const current = val * (x ** power);
      power -= 1;
      return acc + current;
    }, 0);
  }
